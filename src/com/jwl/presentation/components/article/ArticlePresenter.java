...
public class ArticlePresenter extends AbstractPresenter {
	...
	public void renderCreate(){
		...
		container.addAll(new EncodeCreate(this.createFormArticleCreate())
				.getEncodedComponent());
	}

	public void renderEdit(){
		ArticleTO article = (ArticleTO) context.getAttributes().get(
		...
		if(article == null){
			...
			container.addAll(new EncodeEdit(article, form)
					.getEncodedComponent());
		}
	}
	...
	public void renderAdministrationConsole(){
		container.addAll(new EncodeAdministrationConsole()
				.getEncodedComponent());
	}
	...
	public void renderTopicView() throws ModelException{
		...
		container.addAll(new EncodeTopicView(isAnswering, quopteTopicId)
				.getEncodedComponent());
	}
	...
	public void renderArticleSuggestions(){
		...
		IKnowledgeManagementFacade knowledgeFacade = Environment
				.getKnowledgeFacade();
		List<ArticleTO> similarArticles = knowledgeFacade
	...
	public void renderRating() throws ModelException{
		...
		Global.getInstance().getFacade()
				.rateArticle(id, ratedValueNum);
		ArticleTO article = Global.getInstance().getFacade()
				.getArticle(id);
	...
	public void decodeArticleUpdate(){
		try{
			HtmlAppForm form = super.getForm("ArticleEdit");
			ArticleTO article = this.prepareArticle(form);
			article.setId(this.getFacade()
					.findArticleByTitle(article.getTitle()).getId());
	...
	public void decodeArticleDelete(){
		try{
			ArticleId articleId = (ArticleId) super.context.getAttributes()
					.get(JWLContextKey.ARTICLE_ID);
	...
	private List<Integer> getCheckedTopicIds(){
		int elementCheckBoxLength = JWLElements.FORUM_TOPIC_ADMIN_FORM.id
				.length()
